...
LONG_DESCRIPTION = """
## DAG для построения витрины количества землетрясений по дням

### Назначение
Этот DAG агрегирует данные о землетрясениях из слоя ODS (Operational Data Store) в слой DM (Data Mart),
создавая ежедневную витрину количества событий.  
Результат используется для аналитических отчётов, мониторинга сейсмической активности и построения дашбордов.

### Логика работы
1. Ожидание завершения DAG загрузки ODS-слоя (`raw_s3_to_pg`) через сенсор.
2. Создание временной таблицы (staging) с подсчётом количества землетрясений за конкретную дату.
3. Очистка целевой витрины от данных за эту дату.
4. Загрузка агрегированных данных в витрину (`dm.fct_count_day_earthquake`).
5. Удаление временной таблицы.

"""

SHORT_DESCRIPTION = "Агрегация данных о землетрясениях из ODS в витрину DM с ежедневным подсчётом количества событий"
